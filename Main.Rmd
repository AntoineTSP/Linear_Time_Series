...
-   delays of `y` ;
-   delays of `x` ;
-   aggregates of `y` and/or `x` delays.
...
Partial least squares (PLS) regression. The number of components to be retained is determined by cross-validation on the training data. Reference: [cliquer ici](https://www.math.univ-toulouse.fr/~besse/Wikistat/pdf/st-m-app-sparse-pls.pdf) ;

-   PLS than on the delays of the production `y` ;
...
## Forecasts are bound if they are less than 0
...
-   Compare this rmse to the initial forecast ;
-   Add the covariates in `x_` to the model. Is there an improvement? Why or why not?
-   Create more lag aggregates and add them to the model. Is there an improvement?

### LS

-   Implement a linear regression. Compare it to your best PLS model?
-   Reference : [cliquez ici.](https://bookdown.org/evraloui/lbira2110/modeles-lineaires.html)
